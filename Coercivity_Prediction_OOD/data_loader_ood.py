...
for graph_features in data_x:
    for node_features in graph_features:
        edge_length = np.emath.logn(1.05, 1 + node_features[14]/100)
        node_features[0] = node_features[0]*edge_length
        node_features[1] = node_features[1]*edge_length
        node_features[2] = node_features[2]*edge_length
        node_features[3] = node_features[3]*edge_length**3
        node_features[4] = node_features[4]*edge_length
        node_features[7] = node_features[7]*edge_length**2
        node_features.extend([np.linalg.norm([node_features[0],
              node_features[1],node_features[2]])])
...
data_x = [data_x[i] for i in range(len(data_x)) if i not in indices_to_delete]
data_y = [data_y[i] for i in range(len(data_y)) if i not in indices_to_delete]
edge_index_raw = [edge_index_raw[i] for i in range(len(edge_index_raw)) if i not in indices_to_delete]
test_indices = [i for i in range(len(data_x)) if data_x[i][0][14] > 1500] # len(data_x[i]) > 330 and
...
# Filter the dataset for test set
dataset_te = [dataset[i] for i in test_indices]

# Identify remaining indices for training/validation
remaining_indices = [i for i in range(len(dataset)) if i not in test_indices]
remaining_dataset = [dataset[i] for i in remaining_indices]

# Define train and validation split
train_size = int(0.7 * len(remaining_dataset))
val_size = len(remaining_dataset) - train_size  # Remaining data goes to validation

# Perform the split
dataset_tr, dataset_val = random_split(remaining_dataset, [train_size, val_size])
print(f'Length of Train Set: {len(dataset_tr)}, Length of Validation Set: {len(dataset_val)}, Length of Test Set: {len(dataset_te)}')
# --------------------------------------------
