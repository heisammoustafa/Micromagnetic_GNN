...
with open(f"PKL_BHmax/data_x_bigger_bhmax.pkl", "rb") as file:
    data_x_trial = pickle.load(file)
    data_x.extend(data_x_trial)
    
with open(f"PKL_BHmax/data_y_bigger_bhmax.pkl", "rb") as file:
    data_y_trial = pickle.load(file)
    data_y.extend(data_y_trial)
    
with open(f"PKL_BHmax/edge_index_bigger_bhmax.pkl", "rb") as file:
    edge_index_raw_trial = pickle.load(file)
    edge_index_raw.extend(edge_index_raw_trial)

with open(f"PKL_BHmax/data_x_1_V3_bhmax.pkl", "rb") as file:
    data_x_trial = pickle.load(file)
    data_x.extend(data_x_trial)
    
with open(f"PKL_BHmax/data_y_1_V3_bhmax.pkl", "rb") as file:
    data_y_trial = pickle.load(file)
    data_y.extend(data_y_trial)
    
with open(f"PKL_BHmax/edge_index_1_V3_bhmax.pkl", "rb") as file:
    edge_index_raw_trial = pickle.load(file)
    edge_index_raw.extend(edge_index_raw_trial)
    
with open(f"PKL_BHmax/data_x_2_V3_bhmax.pkl", "rb") as file:
    data_x_trial = pickle.load(file)
    data_x.extend(data_x_trial)
    
with open(f"PKL_BHmax/data_y_2_V3_bhmax.pkl", "rb") as file:
    data_y_trial = pickle.load(file)
    data_y.extend(data_y_trial)
    
with open(f"PKL_BHmax/edge_index_2_V3_bhmax.pkl", "rb") as file:
    edge_index_raw_trial = pickle.load(file)
    edge_index_raw.extend(edge_index_raw_trial)
    
with open(f"PKL_BHmax/data_x_coneangle_bhmax.pkl", "rb") as file:
    data_x_trial = pickle.load(file)
    data_x.extend(data_x_trial)
    
with open(f"PKL_BHmax/data_y_coneangle_bhmax.pkl", "rb") as file:
    data_y_trial = pickle.load(file)
    data_y.extend(data_y_trial)
    
with open(f"PKL_BHmax/edge_index_coneangle_bhmax.pkl", "rb") as file:
    edge_index_raw_trial = pickle.load(file)
    edge_index_raw.extend(edge_index_raw_trial)
    
with open(f"PKL_BHmax/data_x_edge_length_var_bhmax.pkl", "rb") as file:
    data_x_trial = pickle.load(file)
    data_x.extend(data_x_trial)
    
with open(f"PKL_BHmax/data_y_edge_length_var_bhmax.pkl", "rb") as file:
    data_y_trial = pickle.load(file)
    data_y.extend(data_y_trial)
    
with open(f"PKL_BHmax/edge_index_edge_length_var_bhmax.pkl", "rb") as file:
    edge_index_raw_trial = pickle.load(file)
    edge_index_raw.extend(edge_index_raw_trial)

# Load _bigger datasets
with open(f"PKL_BHmax/data_x_fisch_2_bhmax.pkl", "rb") as file:
...
with open(f"PKL_BHmax/data_y_fisch_2_bhmax.pkl", "rb") as file:
    data_y_big = pickle.load(file)
    data_y.extend(data_y_big)

with open(f"PKL_BHmax/edge_index_fisch_2_bhmax.pkl", "rb") as file:
...
#idx_te = [i for i in indices_to_delete if data_x[i][0][14]>220]
for graph_features in data_x:
    for node_features in graph_features:
...
for i in range(len(data_x_standardized)):
    ...
    edge_attr = torch.tensor([[node_features[edge[0]][13]] for edge in edge_index_raw[i]], dtype=torch.float)
    node_features = np.delete(node_features, 13, axis=1) 
